...
`GET /s/{shortCode}` 是对外短链地址，`GET /api/v1/redirect/s/{shortCode}` 是兼容跳转入口；两者都会调用解析服务，成功时返回 HTTP 302 和 `Location`。`POST /api/v1/short-links/resolve` 接收完整短链 URL，从其路径最后一段提取短码，并以 JSON 返回解析详情；提取出的短码仍需通过 6 到 8 位 Base62 校验。`POST /api/v1/short-links/broken` 同样接收完整短链 URL 和断链原因，再提取短码执行主动断链。普通短链返回唯一 URL；盲盒先消耗次数，再随机返回 URL；成功解析后在该短链的状态锁内增加访问次数。
...
主动请求用户提供的 URL 存在 SSRF 风险。请求前首先限制协议为 HTTP/HTTPS，并解析 host。除了检查 `localhost` 字符串，还通过 `InetAddress` 检查解析结果，拒绝回环、链路本地、site-local、IPv6 ULA（`fc00::/7`）、组播、未指定地址，以及受限的 IPv4-mapped 地址。受限 IPv4-mapped 地址包括 0/8、10/8、127/8、169.254/16、172.16/12 和 192.168/16。
...
同步请求采用三层容量保护：盲盒最多 10 个候选 URL；`shortUrls` 原始请求列表最多 20 项；展开所有已找到短链后，原始 URL 配置总数最多 32 个。任一规模校验失败都会在提交网络任务前返回 HTTP 400。短链按首次出现顺序去重；同一批次完全相同的原始 URL 也只提交一次探测，再按各短链原有 URL 顺序回填完整 `urlResults`。

整个同步批次共用 8 秒绝对截止时间，而不是逐个 Future 各等待 8 秒。固定 16 个探测线程处理 32 个 URL 最多形成两个波次，单 URL 3 秒时理论最坏探测时间约 6 秒，并为线程调度、资源竞争和结果聚合保留约 2 秒余量。批次截止时，尚未完成的 URL 返回 `health check batch timed out`，整体消息为 `health check incomplete`，并禁止据此自动断链；检测器内部异常同样只影响对应 URL，不中断其他结果。正常网络不可达仍保留完整明细，只有所有候选均不可达且不存在内部失败时，`markBroken=true` 才允许自动断链。
