...
2. 当前值小于等于 0 时失败；
3. 将剩余次数减一；
4. 释放锁后，其他并发请求才能继续判断和扣减。

检查和扣减在同一把锁内执行，因此不会在两者之间留下竞态窗口，也不会造成超发。

只有在锁内成功扣减后才允许随机选择 URL 和增加访问次数。扣减将剩余次数变为 0 后，短链自动标记为 `BROKEN`，断链原因记录为盲盒有效次数耗尽；该次已经取得资格的解析仍会成功返回。后续请求会先因 `BROKEN` 状态被拒绝，并根据剩余次数为 0 返回 `BLIND_BOX_EXHAUSTED`。
